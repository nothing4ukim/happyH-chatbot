...
- **인체도 마크는 텍스트 요약에서 부위 이름으로 자동 판독**해야 한다 —
  EMR 텍스트에 "인체도에 2곳 표시"만 남으면 어디가 아픈지 알 수 없다.
  `bmRegionName(x,y)`: 좌표(%)를 "우측 어깨(앞)"·"허리(뒤)" 식으로 변환.
  x<50=앞모습(마주보므로 그림 왼쪽=환자 **오른쪽**), x≥50=뒷모습(그대로).
  세로 밴드(y%): ~14 머리 / ~18 목 / ~22 어깨 / ~33 가슴·견갑부 / ~39 윗배·등 /
  ~47 아랫배·허리 / ~53 골반·엉덩이 / ~68 허벅지 / ~74 무릎·오금 / ~88 정강이·종아리 / 발.
  중심선 이탈 거리로 몸통 vs 팔(위팔→팔꿈치→전완→손) 구분, 중심 ±2.5%는 좌우 표기 생략.
  표기는 "(자동판독)"을 반드시 붙인다(경계부는 한 칸 옆으로 판독될 수 있음 — 그림이 원본).
  **⚠ 새 병원이 다른 인체도 그림을 쓰면 중심선(행복한H: 앞 29.7% / 뒤 70.3%)과
  세로 밴드를 격자 오버레이로 다시 보정해야 한다** (그림에 10% 격자를 입혀 육안 확인
  → 대표 지점 20여 곳 단위 테스트로 검증).
